use bevy::app::*;
use bevy::prelude::{Added, Component, Deref, DerefMut, Entity, Query, RemovedComponents, ResMut, Resource};
...
impl<T> Plugin for IndexPlugin<T>
    where T: Default + Component + Clone + Ord + Hash + Send + Sync + 'static,
{
    fn build(&self, app: &mut App) {
        app.init_resource::<Index<T>>().add_systems(
            Update,
            (insert_index::<T>, remove_index::<T>),
        );
    }
}
...
fn remove_index<T>(mut index: ResMut<Index<T>>, mut removed_evt: RemovedComponents<T>, reverse_lookup_q: Query<&T>)
where
    T: Component + Clone + Ord + Hash + Send + Sync + 'static,
{
